...
// Simplifying error embedding with hotpotato(https://jatcwang.github.io/hotpotato/)
object HotPotatoErrors {
  ...
  object TodoService {
    trait Service[R] {
      ...
      def finish(id:   Int):    ZIO[R with Store with Logger with TrxHandler, FinishErrorHP, Unit]
    }
...
//          ZIO.accessM(_.store.list)
        ...
        def finish(id: Int) = {
          for {
            _ <- if (id < 0) ZIO.fail(InvalidId(id).embed) else ZIO.succeed(())
            _ <- Logger.>.info(s"Finishing todo: ${id}").refineToOrDie[FinishErrorHP]
            _ <- ZIO
              .accessM[Store with TrxHandler](env => TrxHandler.>.run(env.store.finish(id)))
              .refineToOrDie[FinishErrorHP]
              .flatMap {
                case UpdateResult.Updated  => ZIO.succeed(())
                case UpdateResult.NotFound => ZIO.fail(TodoNotFound(id).embed)
              }
          } yield ()
        }
      }
